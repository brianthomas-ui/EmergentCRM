...
- **Sync architecture**: all URL↔tab logic lives in one never-remounting `components/tabs/TabUrlSync.jsx` (sibling of `<Routes>`), using "what-changed" detection (URL changed → URL wins; active changed → push tab URL) + a `prevLoc` null sentinel. `TabsContext` exposes a `ready` flag (true only after the authed user's tabs load) so the persisted-tab load can't clobber a deep-link tab. This removed the earlier oscillation (root causes: Layout remounting per route reset a one-shot guard; persisted load ran after the deep-link openTab).
- **Browser-style UX**: middle-click a tab to close; **Ctrl/Cmd+W** closes the active (non-pinned) tab; **Ctrl/Cmd+Shift+T** reopens the last-closed tab (closed-tab stack in `TabsContext.reopenLast`). Per-tab + active-tab **copy-link** buttons in `TabStrip` (right-click a tab also copies), with a robust `execCommand` fallback so clipboard denial never throws.
