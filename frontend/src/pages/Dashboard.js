...
function Stat({ label, value, sub, icon: Icon, accent = "text-zinc-900", testid }) {
  return (
    <div
      data-testid={testid}
...
function StageFunnel({ counts }) {
  ...
  return (
    <div className="bg-white border border-zinc-200 rounded-lg p-5">
      <h3 className="font-heading text-base font-bold tracking-tight text-zinc-900 mb-4">
        Pipeline Funnel
      </h3>
        ...
        {order.map((s) => (
          <div key={s} className="flex items-center gap-3">
            ...
            <div className="flex-1 bg-zinc-100 rounded-lg h-6 overflow-hidden">
              <div
                className="h-full bg-zinc-900 rounded-lg transition-all"
                style={{ width: `${((counts[s] || 0) / max) * 100}%` }}
              />
            </div>
            <div className="w-8 text-right text-sm font-semibold text-zinc-900 font-mono">
              {counts[s] || 0}
            </div>
          </div>
        ))}
    ...
    <div className="space-y-6">
      ...
      <div className="bg-white border border-zinc-200 rounded-lg p-5">
        ...
        <div className="bg-zinc-100 rounded-md h-3 overflow-hidden">
          <div
            ...
            className="h-full bg-zinc-950 rounded-md transition-all"
      ...
      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white border border-zinc-200 rounded-lg p-5 flex items-center gap-4">
          ...
          <div>
            <div className="font-heading text-2xl font-bold text-zinc-900">{data.hot}</div>
            <div className="text-xs text-zinc-400 uppercase tracking-widest font-semibold">Hot Leads</div>
          </div>
        </div>
        <div className="bg-white border border-zinc-200 rounded-lg p-5 flex items-center gap-4">
          <CalendarCheck className="w-8 h-8 text-amber-500" />
          <div>
            <div className="font-heading text-2xl font-bold text-zinc-900">{data.follow_up}</div>
        ...
        <div className="bg-white border border-zinc-200 rounded-lg p-5 flex items-center gap-4">
          ...
          <div>
            <div className="font-heading text-2xl font-bold text-zinc-900">
            ...
            <thead>
              <tr className="bg-zinc-50 border-b border-zinc-200">
                {["Agent", "Leads", "Meetings", "Won", "Revenue", "Target %"].map((h) => (
                  <th
                    key={h}
                    className="text-xs font-semibold text-zinc-500 uppercase tracking-widest text-left p-3"
                  >
                    {h}
                  </th>
                ))}
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        <img
                          src={a.avatar_url}
                          alt=""
                          className="w-7 h-7 rounded-lg object-cover border border-zinc-200 bg-zinc-100"
                        />
