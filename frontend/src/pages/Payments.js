...
export default function Payments() {
  ...
  const totalPending = payments.filter((p) => p.payment_status !== "paid").reduce((s, p) => s + p.amount, 0);

  return (
    <div className="space-y-5">
      <div>
        <h1 className="font-heading text-3xl font-black tracking-tighter text-slate-900">Payments</h1>
        <p className="text-sm text-slate-500 mt-1">Stripe (live test) + Razorpay payment links</p>
      </div>

      <div className="grid grid-cols-3 gap-4">
        <div className="bg-white border border-slate-200 rounded-sm p-5">
          <div className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold">Collected</div>
          <div className="font-heading text-3xl font-black text-emerald-600">{money(totalPaid)}</div>
        </div>
        <div className="bg-white border border-slate-200 rounded-sm p-5">
          <div className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold">Pending</div>
          <div className="font-heading text-3xl font-black text-amber-600">{money(totalPending)}</div>
        </div>
        <div className="bg-white border border-slate-200 rounded-sm p-5">
          <div className="text-[10px] uppercase tracking-widest text-slate-400 font-semibold">Links Sent</div>
          <div className="font-heading text-3xl font-black text-slate-900">{payments.length}</div>
        </div>
      </div>
          ...
          <tbody>
            {payments.map((p) => (
              <tr key={p.id} data-testid={`pay-row-${p.id}`} className="border-b border-slate-100 hover:bg-slate-50">
                ...
                <td className="p-3 text-sm font-mono font-semibold text-slate-900">{money(p.amount, p.currency)}</td>
