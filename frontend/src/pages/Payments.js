    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-3 gap-4">
        ...
        <div className="bg-white border border-zinc-200 rounded-lg p-5">
          ...
          <div className="font-heading text-3xl font-bold text-zinc-900">{payments.length}</div>
        </div>
      </div>
                ...
                <td className="p-3">
                  <div className="flex items-center gap-2 justify-end">
                    ...
                    {p.payment_status !== "paid" && p.provider === "stripe" && (
                      <button onClick={() => refresh(p)} className="text-zinc-400 hover:text-zinc-900" title="Refresh status" data-testid={`refresh-${p.id}`}><RefreshCw className="w-3.5 h-3.5" /></button>
                    )}
