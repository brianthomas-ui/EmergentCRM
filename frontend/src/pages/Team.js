  ...
  return (
    <div className="space-y-5">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="font-heading text-3xl font-black tracking-tighter text-slate-900">Team</h1>
          <p className="text-sm text-slate-500 mt-1">{team.length} members · manage agents & targets</p>
        </div>
      ...
      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
        {team.map((m) => (
          <div key={m.id} data-testid={`team-card-${m.id}`} className="bg-white border border-slate-200 rounded-sm p-5">
            <div className="flex items-center gap-3 mb-3">
              <img src={m.avatar_url} alt="" className="w-12 h-12 rounded-sm object-cover border border-slate-200 bg-slate-100" />
              <div>
                <div className="font-heading font-bold text-slate-900">{m.name}</div>
                <div className="text-xs text-slate-400">{m.email}</div>
              </div>
            </div>
            <div className="flex items-center justify-between">
              <Badge className={m.role === "admin" ? "bg-slate-900 text-white border-slate-900" : "bg-sky-50 text-sky-700 border-sky-200"}>
              ...
              {m.role === "agent" && (
                <span className="text-xs text-slate-500 font-mono">Target {money(m.monthly_target)}/mo</span>
              )}
            </div>
          </div>
        ))}
