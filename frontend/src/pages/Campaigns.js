  ...
  return (
    <div className="space-y-5">
      <div className="flex items-end justify-between">
        <div>
          <h1 className="font-heading text-3xl font-bold tracking-tighter text-zinc-900">Campaigns</h1>
          <p className="text-sm text-zinc-500 mt-1">Segmented Gmail mail-merge outreach to existing users</p>
        </div>
      ...
      <div className="grid md:grid-cols-2 gap-4">
        {campaigns.map((c) => (
          <div key={c.id} data-testid={`campaign-${c.id}`} className="bg-white border border-zinc-200 rounded-lg p-5">
            <div className="flex items-start justify-between mb-2">
              <div>
                <h3 className="font-heading text-base font-bold text-zinc-900">{c.name}</h3>
                <div className="text-xs text-zinc-400">{c.segment_label} · spend ≥ ${c.min_spend}</div>
              </div>
              <Badge className={c.status === "sent" ? "text-emerald-600 border-emerald-200" : "text-zinc-500 border-zinc-200"}>{c.status}</Badge>
            </div>
            <div className="text-sm text-zinc-600 mb-3 truncate">{c.template_subject}</div>
            ...
            {c.status !== "sent" ? (
              ...
              <div className="text-xs text-zinc-400 text-center">Sent · {c.recipient_count} recipients</div>
            )}
          </div>
        ))}
        {campaigns.length === 0 && (
          <div className="col-span-2 bg-white border border-zinc-200 rounded-lg p-12 text-center text-zinc-400 text-sm">
...
function Metric({ icon: Icon, label, value }) {
  return (
    <div className="border border-zinc-100 rounded-lg p-2 text-center">
      <Icon className="w-3.5 h-3.5 text-zinc-400 mx-auto mb-1" />
      <div className="text-sm font-bold text-zinc-900 font-mono">{value}</div>
