...
export default function Layout({ children }) {
  ...
  return (
    <div className="min-h-screen bg-slate-50">
      {/* Sidebar */}
      <aside className="w-60 border-r border-slate-200 bg-white h-screen fixed left-0 top-0 flex flex-col z-20">
        <div className="px-5 h-16 flex items-center gap-2 border-b border-slate-200">
          ...
          <div className="leading-tight">
            <div className="font-heading font-extrabold text-sm tracking-tight text-slate-900">
              UPSELL CRM
            </div>
            ...
            .map((item) => {
              ...
              return (
                <NavLink
                  ...
                  className={({ isActive }) =>
                    `flex items-center gap-3 px-3 py-2 rounded-sm text-sm font-semibold transition-colors ${
                      isActive
        ...
        <div className="border-t border-slate-200 p-3">
          <div className="flex items-center gap-3 px-2 py-2">
            <img
              src={user?.avatar_url}
              alt={user?.name}
              className="w-8 h-8 rounded-sm object-cover border border-slate-200 bg-slate-100"
            />
